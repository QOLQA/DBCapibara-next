		...
		>
			...
			<div className="grid gap-1.5">
				{payload.map((item, index) => {
					const itemObj = item as { name?: string; dataKey?: string; value?: number; payload?: Record<string, unknown>; fill?: string; color?: string };
					const key = `${nameKey || itemObj.name || itemObj.dataKey || "value"}`;
					const itemConfig = getPayloadConfigFromPayload(config, item, key);
					const indicatorColor = color || itemObj.payload?.fill || itemObj.color;

					return (
						<div
							key={String(itemObj.dataKey)}
							className={cn(
						...
						>
							{formatter && itemObj?.value !== undefined && itemObj.name ? (
								formatter(itemObj.value, itemObj.name, item, index, Array.isArray(itemObj.payload) ? itemObj.payload : [])
							) : (
								<>
									{itemConfig?.icon ? (
										<itemConfig.icon />
...
function ChartLegendContent({
	...
	return (
		...
		>
			{payload.map((item) => {
				const itemObj = item as { value?: string; dataKey?: string; color?: string };
